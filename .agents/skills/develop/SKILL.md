...
- Stage-handler tests live in `tests/test_workflow_<stage>.py` (`_decomposition`, `_implementing`, `_conflicts`); the validating stage is split into focused `tests/test_workflow_validating_*.py` files (review loops + retry caps, handoff, squash, watermarks, drift, verify, terminal) and the in_review stage into focused `tests/test_workflow_in_review_*.py` files (routing, watermarks, filtering, parked, migration, checks, drift). Facade-level dispatcher / tick / pickup tests stay in `tests/test_workflow.py`. Shared fixtures go in `tests/workflow_helpers.py`.
