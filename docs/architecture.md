...
Local Postgres service intended as the eventual aggregation target for the JSONL sink above. This child ships the **service contract and schema only** — the orchestrator does not yet write to Postgres, and `ANALYTICS_DB_URL` is a reserved env var with no consumer in `orchestrator/config.py`. A follow-up child will add the ingestion path that reads JSONL records and inserts them through this schema.
...
**Endpoint shape.** The eventual consumer reads a single libpq URL — `ANALYTICS_DB_URL` (default `postgresql://orchestrator:orchestrator@127.0.0.1:5432/orchestrator_analytics`) — rather than separate host / port / user / password variables. The endpoint-shaped contract means moving the database off-host later (managed Postgres, a different VM, a unix socket) is a one-line repoint rather than a code change; this child's compose service can be stopped entirely once the orchestrator points at a remote database.

**Schema.** [`../analytics-db/init/01-schema.sql`](../analytics-db/init/01-schema.sql) defines one `analytics_events` table whose columns mirror the JSONL record shape produced by `analytics.build_record`. The common keys (`ts`, `repo`, `issue`, `event`) are `NOT NULL`; everything else is nullable so any single record across the three event kinds (`stage_enter`, `stage_evaluation`, `agent_exit`) is a valid row. An `extras JSONB` column captures any field added to `build_record` before the DDL knows about it, so a JSONL record from a newer orchestrator version can be ingested without losing data; promoted-to-column fields should be removed from `extras` by the ingest job when it learns about them. `source_path` / `source_line` are reserved for the ingest job's own dedup bookkeeping (which JSONL file and byte offset the row came from) so replaying the same log twice can be detected. Indexes cover the expected query dimensions (`ts`, `(event, ts)`, `(repo, issue)`, and a partial index on non-null `stage`). The init script is run by the postgres image once when the data volume is empty (`/docker-entrypoint-initdb.d`); `IF NOT EXISTS` guards keep it idempotent for the operator-driven case (`psql -f` against an existing instance) as well.

**Out of scope for this child.** No orchestrator code reads `ANALYTICS_DB_URL` yet, there is no Postgres client dependency in `pyproject.toml`, and the JSONL sink continues to be the authoritative analytics surface. The follow-up child that wires the ingestion will: pin a Postgres driver in `pyproject.toml`, add `ANALYTICS_DB_URL` to `orchestrator/config.py`, write the JSONL→Postgres replay path, and document the connect / retry / backoff behavior here.
