...
**Skill-trigger follow-ups.** The per-invocation `skill_triggered` audit event on [`EVENT_LOG_PATH`](#audit-event-log-event_log_path) has now shipped (see the [audit event-kinds table](#audit-event-log-event_log_path)) — it is gated on the same `TRACK_SKILL_TRIGGERS` switch and reuses the list `record_agent_exit` already parsed. One follow-up from [`../plans/skill-trigger-tracking.md`](../plans/skill-trigger-tracking.md) remains out of scope: a skill-trigger-rate dashboard widget over the new fields (a pure read-side addition once data accumulates, since `extras JSONB` already stores them with no schema change). The dashboard exposes no skill widget today.
