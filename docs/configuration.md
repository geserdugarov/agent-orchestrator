...
| `TRAJECTORY_LOG_PATH`      | _(unset, off)_                   | opt-in path switch for the trajectory sink — an independent JSONL file for per-run agent reasoning trajectories, separate from `ANALYTICS_LOG_PATH`. Defaults off: unset / empty / `off` / `disabled` / `none` (case-insensitive) all disable it; any other value is the explicit opt-in path. The sink's append / prune / retention mechanics ship today, but no producer is wired yet (the `usage.parse_agent_trajectory` classifier is not connected to `agent_exit`), so it stays inert until that wiring lands. Never touches `ANALYTICS_LOG_PATH`, the analytics Postgres sync, or the dashboard. See [`observability.md#trajectory-sink`](observability.md#trajectory-sink-trajectory_log_path). |
| `TRAJECTORY_RETENTION_DAYS`| `90`                             | retention window for `TRAJECTORY_LOG_PATH`, same semantics as `ANALYTICS_RETENTION_DAYS`: `prune_trajectory_records()` removes older records and `0` (or any non-positive value) keeps trajectories indefinitely. Not yet called from the polling loop, so trajectory retention is operator-driven until the sink is wired. |
