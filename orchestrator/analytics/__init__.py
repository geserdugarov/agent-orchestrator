...
primitives, stage / repo-skill / agent-exit recorders, and the opt-in
trajectory sink) lives in `orchestrator.analytics._recording`; this
`__init__` re-exports its public surface so callers keep importing from
...
the recorders in `_recording` read the values back off this package at
call time (see `_recording._live_settings`), so a patched or reloaded
value takes effect. Each package instance imports its own `_recording`
(the eviction below), bound to that instance, so a reference held across
a reload keeps reading the knobs its own callers patched. `ANALYTICS_DB_URL`
...
is opt-in and defaults *off*. Full sink semantics -- event kinds, record
shapes, redaction, locks, fail-open persistence, retention pruning, and
the trajectory sink -- are documented in `_recording`.
"""
...
# Evict any cached `_recording` so this package instance imports its own copy.
# `tests/test_analytics.py::_reload` pops + re-imports `orchestrator.analytics`
# to land a patched env; a shared `_recording` would bind its `_facade` to
# whichever package instance imported it last, so a stale holder's recorders
# would read the reloaded instance's sink knobs. A per-instance `_recording`
# keeps each side of the reload reading the knobs its own callers patched.
