...
`_handle_documenting` is a thin router over stage-private helpers:
terminal short-circuits (`_finalize_documenting_terminal`,
`_park_documenting_without_pr`), drift detection + unwind
(`_reconcile_documenting_drift`, `_reset_documenting_drift_worktree`),
the parked-no-input fast path (`_documenting_parked_no_input`), worktree
prep (`_prepare_documenting_worktree`), the docs agent run
(`_run_documenting_dev`), and the post-agent disposition
(`_dispose_documenting_outcome`, `_push_docs_and_advance`,
`_route_documenting_no_change`). None of these are re-exported from
`workflow.py`; they are private to this module.
...
def _reset_documenting_drift_worktree(
    ...
    # distinguishable from a real "in sync" result. `_branch_ahead_behind`
    # swallows git errors as `(0, 0)`, which would silently let an
    # unpushed local docs commit against the OLD body survive into the
    # next final-docs hop's recovered-commit shortcut. Use the same git
    # invocation that helper uses but check the exit code + parse here.
    ...
    if ahead is None or behind is None:
        ...
        gh.write_pinned_state(issue, state)
        return False
    # Also reconcile uncommitted edits: a prior docs run may have edited
    # files without committing (parked via `_on_dirty_worktree` /
    # `_on_question` / `agent_timeout`) before the body edit landed.
    # `_worktree_dirty_files` surfaces both modified-tracked AND untracked
    # paths, so treat any non-empty list as a cleanup trigger. Without
    # this the dirty docs edits would ride into the next reviewer round
    # under the new body.
    #
    # And reconcile when the remote PR head moved past local HEAD
    # (`behind > 0`): the reviewer must re-evaluate the actual PR head,
    # not a stale local snapshot of an older commit. Without the reset,
    # the next reviewer round would `git diff` against the un-fetched
    # local HEAD and silently miss commits the remote already has.
    ...
    if ahead > 0 or behind > 0 or dirty:
        ...
        if reset.returncode != 0:
            ...
            gh.write_pinned_state(issue, state)
            return False
        # `git reset --hard` does not remove untracked files, so any
        # untracked docs edits would still survive. `git clean -fd`
        # removes them. The `-d` also clears untracked directories, which
        # matters when the docs agent created new under-`docs/` subdirs
        # that the reviewer never approved.
...
def _reconcile_documenting_drift(
    gh: GitHubClient, spec: RepoSpec, issue: Issue, state: PinnedState,
    branch: str,
) -> bool:
    ...
    # reconcile failed and parked) and nothing fresh has happened since,
    # stay silent. The fast-path check here means the parked state
    # survives operator inspection without re-posting the same park
    # comment every tick. Operator unpark (`awaiting_human=False`) OR new
    # human comments fall through to the unwind-retry block below, which
    ...
    # Set/keep the drift-unwind sentinel. The marker survives every park
    # inside this path, so an operator unpark or a later human comment
    # (without a fresh drift) re-enters here on the next tick and retries
    # the reconcile + relabel. The marker is cleared ONLY on the success
    # path that relabels to `validating`; without it, an operator unpark
    # on a failed reconcile would fall through to the normal flow and
    # (via the recovered-commit shortcut or a fresh docs spawn) advance
    # to `in_review` against the OLD body -- skipping the required
    # `validating` re-review.
    state.set("docs_drift_unwind_pending", True)
    state.set("awaiting_human", False)
    state.set("park_reason", None)
    # Clear `review_round` BEFORE any fallible cleanup (fetch / reset).
    # Drift means the prior reviewer approval is stale regardless of
    # whether the on-disk reset succeeds, so the round counter must drop
    # now. If we park on fetch failure inside the reconcile, an operator
    # unpark or manual relabel must not be able to ride the stale
    # approval into a new final-docs handoff that skips the required
    # re-review.
    state.set("review_round", 0)
    wt = _wf._worktree_path(spec, issue.number)
    if wt.exists():
        if not _reset_documenting_drift_worktree(
            gh, spec, issue, state, branch, wt,
        ):
            return True
    # Reconcile succeeded (or the worktree didn't exist): the drift
    # unwind is complete, clear the sentinel and relabel.
...
def _prepare_documenting_worktree(
    gh: GitHubClient, spec: RepoSpec, issue: Issue, state: PinnedState,
    branch: str, wt,
):
...
def _run_documenting_dev(
    gh: GitHubClient, spec: RepoSpec, issue: Issue, state: PinnedState,
    branch: str, wt, ahead: int,
):
    """Run the docs pass and return its result for disposition.
    ...
    Returns `(wt, result, before_sha, recovered, paused)`, or None when an
    ...
    if state.get("awaiting_human"):
        ...
        # builds the followup from ONLY the new human comments, which is
        # the right shape for implementing/validating (the dev has an
        # in-context docs spec already) but wrong for documenting: a
        # `fetch_failed` / `agent_timeout` / `agent_silent` resume may be
        # the FIRST time this session sees the docs-stage instructions
        # (the DOCS: NO_CHANGE marker, what files to inspect, what to
        # commit). Without those, the dev could emit a stray
        # `DOCS: NO_CHANGE` it learned from an earlier spawn and the issue
        # would advance to validating without ever running a real docs
        # pass. `_build_documentation_prompt` quotes the issue body AND
        ...
        # only if) the resumed dev produced a new commit.
        # `_resume_dev_with_text`'s `_ensure_worktree` fallback is
        # harmless here because we already restored the PR-anchored
        # worktree above.
        before_sha = _wf._head_sha(wt)
        # Persist `docs_checked_sha` BEFORE the spawn for the same reason
        # the fresh-spawn shape does: a no-change verdict on this resume
        # relies on this watermark to identify what commit the dev
        # confirmed.
        ...
        # Recovered worktree: a previous tick committed docs but crashed
        # before the push. Build a synthetic result and fall through to
        # the unified commit/dirty/push disposition so an uncommitted file
        # left alongside the recovered commit parks via
        # `_on_dirty_worktree` instead of being silently dropped by the
        # push (which only ships staged work). A drift event this tick
        # would have routed back to `validating` before reaching this
        # shape, so the recovered commit is always against the still-valid
        # approved body.
        _wf.log.info(
        ...
        # Empty `before_sha` makes the post-spawn check treat the
        # recovered HEAD as a fresh commit. `docs_checked_sha` is updated
        # to the recovered HEAD on a successful push.
        before_sha = ""
        recovered = True
        ...
        before_sha = _wf._head_sha(wt)
        state.set("docs_checked_sha", before_sha or "")
        # Persist the spec so a backend hiccup that yields no session id
        ...
        # shape. A direct resume replays the whole transcript every tick
        # without charging the resume budget, so a long-lived session could
...
def _push_docs_and_advance(
    gh: GitHubClient, spec: RepoSpec, issue: Issue, state: PinnedState,
    wt, branch: str, pr_number, after_sha: str, notice: str,
) -> None:
    ...
    try:
        ...
        _wf.log.exception(
            "issue=#%s could not post docs-push notice to PR #%s",
...
def _route_documenting_no_change(
    gh: GitHubClient, spec: RepoSpec, issue: Issue, state: PinnedState,
    wt, branch: str, pr_number, ahead: int, after_sha: str, body: str,
) -> None:
    ...
    if ahead > 0:
        ...
        # (push_failed / agent_timeout / dirty). The resumed dev added
        # nothing new and confirmed no further change is needed, but the
        # requirement is to advance only after the docs commit is PUSHED
        # or a true no-change verdict is parsed for the REMOTE PR state.
        # The local-only commit must reach the remote first; otherwise the
        # reviewer agent at validating would never see the docs in the
        # diff.
        _push_docs_and_advance(
            gh, spec, issue, state, wt, branch, pr_number, after_sha,
            ":books: documenting pass: pushed recovered docs "
            "commit(s) after no-change confirmation.",
        )
        return
    # Persist the SHA the dev evaluated even on a "nothing changed"
    # outcome. The fresh-spawn shape writes `docs_checked_sha =
    # before_sha` BEFORE the spawn (so a no-change outcome there leaves it
    # correct), and the awaiting-human resume path uses the same pre-spawn
    # write; setting it here too makes the post-condition explicit and
    # covers the case where neither pre-spawn write fired (e.g. a future
    # entry path that bypasses them). `after_sha == before_sha` in this
    # branch by construction (no commit means no SHA delta).
    state.set("docs_checked_sha", after_sha)
...
def _dispose_documenting_outcome(
    ...
    # no-change / on_question paths would silently leave docs edits behind
    # on disk that the eventual reviewer never sees. Check before any
    # other decision so an agent that edited files without committing (and
    # then either emitted `DOCS: NO_CHANGE`, asked a question, or produced
    ...
    # Unknown verdict: either a real question, an ambiguous message, or an
    # empty / silent run. Reuse `_on_question`, which posts the HITL ping,
...
def _handle_documenting(gh: GitHubClient, spec: RepoSpec, issue: Issue) -> None:
    ...
    if _reconcile_documenting_drift(gh, spec, issue, state, branch):
        return
    ...
    ahead = _prepare_documenting_worktree(gh, spec, issue, state, branch, wt)
    if ahead is None:
        return
    ...
    # mid-flight has no trustworthy result (the recovered `ahead > 0`
    # shape synthesizes its own non-interrupted result, so only a real
    # resume / fresh-docs spawn can land here). Ignore it and return
    # WITHOUT writing pinned state -- the pre-spawn `docs_checked_sha` /
    ...
    # resume helper already made (the recovered `ahead > 0` shape ran no
    # agent and reports False). Stop before the disposition posts a PR
    # comment, pushes, advances to `in_review`, or writes pinned state.
    # The committed docs work stays on the branch and republishes through
    # the recovered-worktree path once the label is removed.
    if paused:
        return
