...
"""Reconstruct agent run trajectories from JSONL stdout.
...
-- lives in the private ``_usage_metrics`` module, and the skill-trigger
parsing -- the ``SkillTriggers`` dataclass and the ``parse_claude_skills`` /
``parse_codex_skills`` / ``parse_agent_skills`` trio -- lives in the private
``_usage_skills`` module. This module re-exports both public surfaces
unchanged and hosts the trajectory classifier, which reuses their shared event
iterator, token decoders, price path, init-frame helpers, and resilience
contract.
...
# Shared event iterator, token decoders, and price path the trajectory
# classifier below reuses (keeping the resilience contract and cost path
# defined once, in ``_usage_metrics``).
from orchestrator._usage_metrics import (
