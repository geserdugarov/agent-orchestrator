...
- `tests/` — pytest suite. In-memory fakes in `tests/fakes.py`. Stage-handler tests in `tests/test_workflow_<stage>.py` (the validating stage is split across `tests/test_workflow_validating_*.py`, the in_review stage across `tests/test_workflow_in_review_*.py`); facade-level dispatcher / tick / pickup tests in `tests/test_workflow.py`; shared helpers in `tests/workflow_helpers.py`.
