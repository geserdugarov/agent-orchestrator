...
- `the-foundry/src/foundry/stages/verify.py` (function
  `_detect_verify_commands`) auto-detects `verify_commands` from
  `pyproject.toml` (`uv run ruff check .`, plus `uv run pytest -x
  --no-header -q` when a `tests/` directory exists), `package.json`
  (`npm ci` plus whichever of `build` / `lint` / `test` scripts are
  declared; sub-packages walked via `*/package.json`), and `Cargo.toml`
  (`cargo test`). Other languages — `go.mod` / `Gemfile` / etc. — are
  **not** detected upstream; an operator covers them via the
  `VERIFY_COMMANDS` override. VERIFY short-circuits on nonzero return
  code; the LLM reviewer only runs when all commands pass. Our own
  proposed `_handle_verifying` (next section) widens this matrix to
  cover Rust clippy / Go vet, but those go beyond what foundry ships
  today.
...
- Codex CLI exposes hooks as a feature flag: `codex exec --enable
  codex_hooks` (equivalent to `-c features.codex_hooks=true`), with the
  hook script configured under `~/.codex/config.toml`. This is the
  exact surface `blast-furnace`'s stop-hook QG proposal targets. The
  feature is gated rather than always-on, so the orchestrator must
  pass the `--enable codex_hooks` flag at spawn time.
...
Add a `Produced-By:` trailer to dev-agent commits. The agent — not
`agents.py` — runs `git commit` inside the worktree, so the trailer
has to land via a two-step enforcement:
1. **Prompt instruction.** Extend the implementer prompt to require a
   `Produced-By: <backend>` / `Produced-Model: <model>` trailer on
   every commit (the prompt already pins Conventional Commit style;
   this is one more rule).
2. **Orchestrator-owned amendment.** Best-effort backstop in
   `_push_branch` (and inside `_squash_and_force_push` when squash is
   on): before the push, walk the new commits and, for any commit
   missing the trailer, run `git commit --amend --no-edit
   --trailer "Produced-By=<backend>" --trailer "Produced-Model=<model>"`
   for the top commit, or `git filter-branch --msg-filter` /
   `git rebase --exec` for the range. The squash path is the easy
   case: the squash helper already builds the squashed commit's body
   from the original subjects (`_squash_and_force_push`), so adding
   the trailer there is a one-line addition.

The reviewer can then read the trailer with `git log -1 --format=%B`
and confirm the backend matches. Verification on the reviewer side is
read-only so it doesn't need orchestrator ownership.
...
- Codex: `-c, --config <key=value>` overrides
  (`codex exec -c sandbox_permissions='["disk-full-read-access"]' ...`)
  or a named `--profile <name>` referencing a block in
  `~/.codex/config.toml`. The Codex CLI does not expose a
  `--config-toml` flag; per-key overrides via `-c` are the right
  surface.
...
each commit makes `git log` legible. Mechanism is the same two-step
"prompt instruction + orchestrator-owned `git commit --amend
--trailer` in `_push_branch` / `_squash_and_force_push`" from #3 —
agents.py does not own the agent's `git commit` invocation, so the
amend is the enforceable backstop. ~30 lines including the amend
helper.
...
- `the-foundry/src/foundry/{security,state,workflows}.py`,
  `src/foundry/stages/{verify,agent_verify,context,plan,implement,pr}.py`,
  `.env.example`, `src/foundry/agents/prompts/verify.md`.
...
- `iron-press/src/sdk/workflow/permission-profiles.ts`,
  `src/workflows/sm/workflow.json`,
  `src/sdk/workflow/dynamic-loader.ts`,
  `iron-press.config.json.example`.
...
- `blast-furnace/docs/{orchestrator-target-state-plan,
  ...
  `src/jobs/{develop,quality-gate-runner}.ts`.
